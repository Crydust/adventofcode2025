...
//        List<String> lines = readInputLines("/example.txt");
        ...
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < columns; x++) {
                if (grid.isRoll(x, y) && grid.countNeighbors(x, y) < 4) {
                    count++;
                }
            }
        }
    ...
    private static List<String> readInputLines(String name) throws IOException {
        try (var in = new BufferedReader(new InputStreamReader(requireNonNull(Day4Part1.class.getResourceAsStream(name)), UTF_8))) {
