...
//        int shortestN = 10;
        ...
        int result = 1;
        for (int i = 0; i < 3; i++) {
            int size = circuits.reversed().get(i).size();
            result *= size;
        }
    ...
    private static List<String> readInputLines(String name) throws IOException {
        try (var in = new BufferedReader(new InputStreamReader(requireNonNull(Day8Part1.class.getResourceAsStream(name)), UTF_8))) {
